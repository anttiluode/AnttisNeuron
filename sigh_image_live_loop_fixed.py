...
class LiveCollapseApp:
    ...
    # Used only before the window has been laid out for the first time.
    FALLBACK_PANEL_SIZE = (420, 260)
    MIN_PANEL_SIZE = 24
    ...
    def setup_gui(self):
        ...
        eq_frame.pack(fill=tk.X, pady=4)
        ...
        self.eq_widget.pack(padx=4, pady=3, anchor="center")
        ...
        loop_frame.pack(fill=tk.X, pady=4)
        ...
        for r in range(2):
            display.grid_rowconfigure(r, weight=1, uniform="panels")
        for c in range(2):
            display.grid_columnconfigure(c, weight=1, uniform="panels")

        self.original_panel = self._make_panel(
            display, "Original", 0, 0
        )
        self.current_panel = self._make_panel(
            display, "Live recursive state", 0, 1
        )
        self.spectrum_panel = self._make_panel(
    ...
    def _make_panel(self, parent, title, row, col):
        """
        A panel whose image is sized to the space the grid gives it.

        pack_propagate(False) is the key line: without it the image label
        pushes the LabelFrame to the image's size, the grid runs out of
        vertical room, and Tk silently clips the bottom of every panel.
        """
        ...
        label.bind(
            "<Configure>",
            lambda _event, p=panel: self._on_panel_resize(p),
        )
        return panel

    def _on_panel_resize(self, panel):
        """Debounced re-render so dragging the window stays smooth."""
        if panel["resize_job"] is not None:
            try:
                self.root.after_cancel(panel["resize_job"])
            except Exception:
                pass

        panel["resize_job"] = self.root.after(
            80, lambda p=panel: self._resize_panel(p)
        )

    def _resize_panel(self, panel):
        panel["resize_job"] = None

        size = self._panel_target_size(panel)
        if size == panel["last_size"]:
            return

        self._render_panel(panel)

    def _panel_target_size(self, panel):
        label = panel["label"]
        width = label.winfo_width() - 4
        height = label.winfo_height() - 4

        if width < self.MIN_PANEL_SIZE or height < self.MIN_PANEL_SIZE:
            # Not laid out yet; the <Configure> binding will correct this.
            return self.FALLBACK_PANEL_SIZE
    ...
    def _update_display(self, panel, data, colormap=None):
        """Store the array on the panel, then draw it at the current size."""
        panel["data"] = np.asarray(data)
    ...
    def _render_panel(self, panel):
        ...
        panel["label"].config(image=img)
        panel["label"].image = img
        panel["last_size"] = size

    def numpy_to_tkimage(self, array, size, colormap=None):
        if array.ndim == 3 and array.shape[2] == 3:
            ...
            if colormap is None:
                ...
                mapped = _get_cmap(colormap)(norm)[:, :, :3]
        ...
        # Fit inside the panel, preserving aspect ratio, so nothing is
